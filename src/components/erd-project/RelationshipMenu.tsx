...
export function RelationshipMenu() {
  const { lastRelation: relation, menuRef, contextMenu, closeContextMenu } = useMappingContext();
  ...
  const fromMultiplicity =
    relation && relation.participation && relation.participation.from && relation.participation.from === 'PARTIAL'
      ? 'FULL'
      : 'PARTIAL';

  const toMultiplicity =
    relation && relation.participation && relation.participation.to && relation.participation.to === 'PARTIAL'
      ? 'FULL'
      : 'PARTIAL';

  const handleMenuItemClick = (action: 'delete' | 'identify' | 'fromNullable' | 'toNullable') => {
    if (!relation) return;
  ...
  return (
    <styles.menu ref={menuRef} $pos={{ left: contextMenu.x, top: contextMenu.y }}>
      <styles.menuItem onClick={() => handleMenuItemClick('delete')}>관계 삭제</styles.menuItem>
